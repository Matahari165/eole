...
create table public.user_settings (
  user_id uuid primary key default public.current_user_id() references neon_auth."user"(id) on delete cascade,
...
create table public.sessions (
  ...
  user_id uuid not null default public.current_user_id() references neon_auth."user"(id) on delete cascade,
...
create policy "profiles_update_own" on public.profiles for update to authenticated
  using ((select public.current_user_id()) = user_id) with check ((select public.current_user_id()) = user_id);
...
create policy "sessions_delete_own" on public.sessions for delete to authenticated
  using ((select public.current_user_id()) = user_id);
create policy "rounds_select_own" on public.rounds for select to authenticated
  using ((select public.current_user_id()) = user_id);
create policy "rounds_insert_own" on public.rounds for insert to authenticated
  with check (
    (select public.current_user_id()) = user_id
    and exists (
      select 1 from public.sessions
      where sessions.id = session_id
        and sessions.user_id = (select public.current_user_id())
    )
  );
...
begin
  ...
  values (owner_id, trim(p_first_name), lower(trim(p_username)))
...
revoke all on function public.initialize_user_profile(text, text) from public;
...
grant execute on function public.initialize_user_profile(text, text) to authenticated;
